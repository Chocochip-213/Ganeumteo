...
[규모·부담금 가이드 — 권장이며 강제순서 아님, 네 자율 판단]
 ...
 · 부담금: 농지전용(지목 전·답·과수원)이면 law_article_fetch('농지법 시행령','53')로 율(진흥30/밖20)을 읽어 levy_estimate('농지보전부담금', land_price=공시지가, area_m2=대지면적, rate_pct=그 율). 산지전용이면 levy_estimate('대체산림자원조성비')(단가 없음→확인필요). 개발행위 대상이면 levy_estimate('개발부담금')(설계前 금액 불가→부과대상만).
