    ...
    if _tiered:   # 단독/공동주택 등 계단식(별표1): 기본대수 + 임계초과분/증분, 비고6
        ...
        if not prior_evidence_id or prior_evidence_id not in _eids:   # 변경전 기준도 근거 실재(날조 차단)
            return Command(update={"parking_req": {"status": "확인필요", "note": "변경전 기준면적 근거(prior_evidence_id) 미동반/미실재 — 별표1 fetch 후 재호출"},
                                   "_toolcalls": ["parking_quota"],
                                   "messages": [_tm("부설주차 확인필요: 변경전 용도 기준면적 근거 필요(비고5/7 차감)", tool_call_id)]})
...
def get_building_register(pnu: str, tool_call_id: Annotated[str, InjectedToolCallId]) -> Command:
    ...
    if b is True:
        _ext = "".join([
            ...
            f", 내진설계 {r.get('내진설계적용')}" + (f"(내진능력 {r.get('내진능력')})" if r.get('내진능력') else "") if r.get('내진설계적용') else "",
            f", 에너지효율등급 {r.get('에너지효율등급')}" if r.get('에너지효율등급') else "",
            f", 녹색건축등급 {r.get('녹색건축등급')}" if r.get('녹색건축등급') else "",
            f", 승용승강기 {r.get('승용승강기수')}대" if r.get('승용승강기수') else "",
            f", 옥내기계식주차 {r.get('옥내기계식주차대수')}대" if r.get('옥내기계식주차대수') else "",
        ])
        msg = f"기존 건물 있음 — 주용도 {r.get('주용도')}{_ext}, 연면적 {r.get('연면적')}㎡, 지상 {r.get('지상층수')}층, 동수 {r.get('동수')} → 신축 아님(용도변경/대수선 가능성). 빈땅으로 단정 금지. (as-built 건폐율·용적률·사용승인일·구조는 증축여력·기존불일치 판단 사실; 내진설계·에너지효율등급·승강기·기계식주차는 증축/대수선시 내진보강·에너지절약계획서·승강기 사용전검사 의제 판단근거 — 전부 네가 해석, 코드는 표면화만)"
