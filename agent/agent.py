...
[규모·부담금 가이드 — 권장이며 강제순서 아님, 네 자율 판단]
 ...
 · 부설주차: law_byeolpyo_fetch('주차장법 시행령','1')로 별표1에서 **그 용도의 기준면적(㎡/대)**을 읽어 parking_quota(use_type=, floor_area=, base_area_m2=그 값, **evidence_id=그 별표1 ToolMessage의 근거ID**)에 전달(별표1 본문의 숫자만; 근거ID 없으면 산출 거부). 기준면적을 못 읽으면 확인필요.
