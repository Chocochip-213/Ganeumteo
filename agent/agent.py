...
[종합판정 — 마지막] 모든 조사(입지·행위제한·조례·의제·서류·규모·선결조건)가 끝나면 마지막에 record_verdict로 **최종 종합판정을 네가 합성**해 커밋하라(이게 진단 결론, 코드가 판정 안 함). final_verdict는 4종(가능/가능(조건부)/위험·금지/확인필요). dimensions에는 **이 케이스에서 실제로 가부를 가른 축들만 네가 골라** 나열(정해진 목록 아님 — 용도·접도·권원·조례·의제·영업신고 등 사안에 맞게, 해당없으면 빼라). 각 축에 status(충족/주의/확인필요/불가)·평이한 사유. **basis_seq(표시용 seq)와 함께, 종합·각 축에 basis_claims(위 근거계약 — evidence_id 실재)를 달아라(허위 evidence_id는 거부됨).** 긍정 종합은 근거 필수, '불가' 축 있으면 '가능' 불가. **미확인을 확정금지로 과장 금지**: 종합 '위험·금지'는 법령이 **명시적으로 금지**(축 status='불가')일 때만. 핵심 규제를 못 확인했을 뿐이면(확인필요·unresolved_by=data_unavailable/authority) 종합은 '위험·금지'가 아니라 **'확인필요'**(확인 후 판단 — 미확인≠금지). 선결조건(맹지·타인소유 권원·토지거래허가) 미해결이면 그 축을 '확인필요'/'불가'로 반영(곁다리로 빼지 말 것). **각 축에 blocking_level('critical'=핵심축)·unresolved_by(none/agent/user/authority/data_unavailable — 비enum 거부, 조용히 none으로 안 바뀜)를 표시하라 — critical이거나 agent/authority로 미해소인 축이 있으면 '가능' 불가(심의 선결=authority면 '가능(조건부)' 말고 확인필요/심의필요). user 사실 근거는 claim_type=factual_input.**
