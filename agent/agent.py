...
[규모·부담금 가이드 — 권장이며 강제순서 아님, 네 자율 판단]
 ...
 · 부설주차: law_byeolpyo_fetch('주차장법 시행령','1')로 별표1에서 **그 용도의 기준면적(㎡/대)**을 읽어 parking_quota(use_type=, floor_area=, base_area_m2=그 값, **evidence_id=그 별표1 ToolMessage의 근거ID**)에 전달(별표1 본문의 숫자만; 근거ID 없으면 산출 거부). 기준면적을 못 읽으면 확인필요. **용도변경이면** 변경 *전* 용도의 기준면적도 같은 별표1서 읽어 prior_base_area_m2(+prior_evidence_id=그 근거ID)로 함께 전달하라 — parking_quota가 비고5(변경부분만)·비고7(순증 1대 미만→0)으로 *순증분*만 산정한다(신축식 시설면적÷기준 과대산정 금지). 변경 전 용도 기준면적을 못 읽으면 그 사실을 사유에 명시.
