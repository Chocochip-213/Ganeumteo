...
[docs_for_stage 호출법] stage_key는 반드시 실제 단계명만: '건축허가','착공신고','사용승인' 3개 + record_uijae로 기록한 의제의 stage_key 각각(농지전용/산지전용/개발행위). '의제단계' 같은 placeholder 문자열 금지. 의제 없으면 건축허가·착공신고·사용승인 3개만. 호출 시 author_note에 그 단계 작성주체를 법령근거로 한 줄 써라(신청인 본인 / 설계도서=건축사 §23 / 감리보고서=감리자 §25 등 — 네가 법으로 판단, 키워드 추측 금지).
