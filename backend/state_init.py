...
def fresh_state(address, use_type, floor_area=None, floor_count=None):
    ...
    return {
        ...
        "reg_overlaps": [], "uijae": [], "documents": [], "cond_assessments": [], "reg_effects": [], "jorye_verdicts": [], "verdict_labels": [], "document_facts": {}, "levies": [], "_delegated": False, "doc_index_hit": False, "envelope": {},
        # 주: 단일값 채널(zone·pnu·road_side·scale_limits 등)은 _keep_last/_keep_truthy_first 리듀서화해 병렬 동시쓰기 충돌 차단하되, **pre-init 안 함** —
        # 리듀서가 None 첫-write 안전 처리하고, stub/라우팅의 `"X" not in state` 게이트(미설정=아직 안 함)를 보존해야 하기 때문(envelope은 'X not in state' 게이트 없어 pre-init 유지).
