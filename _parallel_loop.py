...
def one(it, gen, persona, judge):
    ...
    try:
        ...
        if any(k in str(e) for k in ("usage limit", "Too Many Requests", "rate limit", "429", "onnection", "refused", "timed out", "Timeout")):
            time.sleep(600)   # 프록시 쿼터소진/다운(Connection error)/타임아웃 — EXC 스핀(프록시 hammering) 대신 백오프, 복구 자동 대기
    with loglock:
