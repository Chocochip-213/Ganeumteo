...
const SRC_KO = { vworld: "정부 공간정보(VWorld)", law: "국가법령", ordin: "자치법규(조례)", data: "공공데이터" };
function srcKo(s) { return SRC_KO[s] || s || "근거"; }   // 소스 원시명 → 사용자 친화 표기
...
function discloseBody(steps, cits) {
  ...
  const citRows = cits.map((c) => {
    ...
    const badge = c.extract_method ? `<span class="bsrc bsrc-${c.source === "vworld" ? "site" : "law"}">${esc(srcKo(c.source))} · ${esc(c.extract_method)}</span>` : `<span class="bsrc bsrc-${c.source === "vworld" ? "site" : "law"}">${esc(srcKo(c.source))}</span>`;
